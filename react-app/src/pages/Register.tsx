...
// Icônes SVG inline (couleur via currentColor)
...
export default function Register() {
  ...
  const isSuccess = message?.toLowerCase().includes('réuss') ?? false;
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <form onSubmit={onSubmit} noValidate style={styles.form}>
          ...
          <div style={styles.formGroup}>
            <label style={styles.label}>
              ...
              <div style={styles.inputWrapper}>
                <span style={{ ...styles.inputIcon, color: focus.confirmPassword ? '#667eea' : '#a0aec0' }} aria-hidden>
                  <IconLock />
                </span>
                <input
                  style={{ ...styles.input, ...(focus.confirmPassword ? styles.inputFocus : undefined) }}
                  type="password"
                  name="confirmPassword"
                  value={form.confirmPassword}
                  onChange={onChange}
                  onFocus={() => setFocus(s => ({ ...s, confirmPassword: true }))}
                  onBlur={() => setFocus(s => ({ ...s, confirmPassword: false }))}
                  placeholder="••••••••"
                  required
                />
              </div>
        ...
        <div style={{ marginTop: 24, fontSize: 14, color: '#718096', textAlign: 'center' }}>
          ...
          <Link
            to="/login"
            style={{ ...styles.link, ...(hoverLink ? styles.linkHover : undefined) }}
